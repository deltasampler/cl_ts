/*
    functions:
        creation:
            vec4(x: number = 0.0, y?: number, z?: number, w?: number): vec4_t
            vec4_new(): vec4_t
            vec4_x(x: number): vec4_t
            vec4_xyzw(x: number, y: number, z: number, w: number): vec4_t
            vec4_clone(v: vec4_t): vec4_t
        setter:
            vec4_zero(v: vec4_t): vec4_t
            vec4_set(v: vec4_t, x: number, y: number, z: number, w: number): vec4_t
            vec4_copy(a: vec4_t, b: vec4_t): vec4_t
        color:
            rgba(r: number = 0.0, g: number = 0.0, b: number = 0.0, a: number = 0.0): vec4_t
            hexa(h: number): vec4_t
        unary:
            vec4_neg(v: vec4_t): vec4_t
            vec4_abs(v: vec4_t): vec4_t
            vec4_inv(v: vec4_t): vec4_t
        arithmetic vector x vector:
            vec4_add(a: vec4_t, b: vec4_t, out: vec4_t): vec4_t
            vec4_add1(a: vec4_t, b: vec4_t): vec4_t
            vec4_add2(a: vec4_t, b: vec4_t): vec4_t
            vec4_sub(a: vec4_t, b: vec4_t, out: vec4_t): vec4_t
            vec4_sub1(a: vec4_t, b: vec4_t): vec4_t
            vec4_sub2(a: vec4_t, b: vec4_t): vec4_t
            vec4_mul(a: vec4_t, b: vec4_t, out: vec4_t): vec4_t
            vec4_mul1(a: vec4_t, b: vec4_t): vec4_t
            vec4_mul2(a: vec4_t, b: vec4_t): vec4_t
            vec4_div(a: vec4_t, b: vec4_t, out: vec4_t): vec4_t
            vec4_div1(a: vec4_t, b: vec4_t): vec4_t
            vec4_div2(a: vec4_t, b: vec4_t): vec4_t
        arithmetic vector x scalar:
            vec4_adds(v: vec4_t, s: number, out: vec4_t): vec4_t
            vec4_adds1(v: vec4_t, s: number): vec4_t
            vec4_adds2(v: vec4_t, s: number): vec4_t
            vec4_subs(v: vec4_t, s: number, out: vec4_t): vec4_t
            vec4_subs1(v: vec4_t, s: number): vec4_t
            vec4_subs2(v: vec4_t, s: number): vec4_t
            vec4_muls(v: vec4_t, s: number, out: vec4_t): vec4_t
            vec4_muls1(v: vec4_t, s: number): vec4_t
            vec4_muls2(v: vec4_t, s: number): vec4_t
            vec4_divs(v: vec4_t, s: number, out: vec4_t): vec4_t
            vec4_divs1(v: vec4_t, s: number): vec4_t
            vec4_divs2(v: vec4_t, s: number): vec4_t
        arithmetic vector x vector x scalar:
            vec4_addmuls(a: vec4_t, b: vec4_t, s: number, out: vec4_t): vec4_t
            vec4_addmuls1(a: vec4_t, b: vec4_t, s: number): vec4_t
            vec4_addmuls2(a: vec4_t, b: vec4_t, s: number): vec4_t
        product:
            vec4_dot(a: vec4_t, b: vec4_t): number
            vec4_cross(a: vec4_t, b: vec4_t): vec4_t
            vec4_cross1(a: vec4_t, b: vec4_t): vec4_t
        norm:
            vec4_len(v: vec4_t): number
            vec4_len_sq(v: vec4_t): number
            vec4_dist(a: vec4_t, b: vec4_t): number
            vec4_dist_sq(a: vec4_t, b: vec4_t): number
            vec4_unit(v: vec4_t, out: vec4_t): vec4_t
            vec4_unit1(v: vec4_t): vec4_t
            vec4_unit2(v: vec4_t): vec4_t
            vec4_dir(a: vec4_t, b: vec4_t, out: vec4_t): vec4_t
            vec4_dir1(a: vec4_t, b: vec4_t): vec4_t
        transform:
            vec4_transf_mat4(v: vec4_t, m: mat4_t, out: vec4_t): vec4_t
            vec4_transf_mat4_1(v: vec4_t, m: mat3_t): vec4_t
            vec4_transf_mat4_2(v: vec4_t, m: mat3_t): vec4_t
        angular:
            vec4_angle(v: vec4_t): vec2_t
            vec4_angle_ab(a: vec4_t, b: vec4_t): number
        interpolation:
            vec4_lerp(a: vec4_t, b: vec4_t, t: number, out: vec4_t): vec4_t
            vec4_lerp1(a: vec4_t, b: vec4_t, t: number): vec4_t
            vec4_lerp2(a: vec4_t, b: vec4_t, t: number): vec4_t
        random:
            vec4_rand(out: vec4_t): vec4_t
            vec4_rand1(): vec4_t
            vec4_rand_unit(scale: number, out: vec4_t): vec4_t
            vec4_rand_unit1(scale: number, out: vec4_t): vec4_t
        rounding:
            vec4_floor(v: vec4_t, out: vec4_t): vec4_t
            vec4_floor1(v: vec4_t): vec4_t
            vec4_ceil(v: vec4_t, out: vec4_t): vec4_t
            vec4_ceil1(v: vec4_t): vec4_t
            vec4_round(v: vec4_t, out: vec4_t): vec4_t
            vec4_round1(v: vec4_t): vec4_t
            vec4_trunc(v: vec4_t, out: vec4_t): vec4_t
            vec4_trunc1(v: vec4_t): vec4_t
            vec4_snap(v: vec4_t, g: vec4_t, out: vec4_t): vec4_t
            vec4_snap1(v: vec4_t, g: vec4_t): vec4_t
        bounding:
            vec4_min(a: vec4_t, b: vec4_t, out: vec4_t): vec4_t
            vec4_min1(a: vec4_t, b: vec4_t): vec4_t
            vec4_max(a: vec4_t, b: vec4_t, out: vec4_t): vec4_t
            vec4_max1(a: vec4_t, b: vec4_t): vec4_t
            vec4_clamp(v: vec4_t, min: vec4_t, max: vec4_t, out: vec4_t): vec4_t
            vec4_clamp1(v: vec4_t, min: vec4_t, max: vec4_t): vec4_t
            vec4_clamp2(v: vec4_t, min: vec4_t, max: vec4_t): vec4_t
            vec4_wrap(v: vec4_t, min: vec4_t, max: vec4_t, out: vec4_t): vec4_t
            vec4_wrap1(v: vec4_t, min: vec4_t, max: vec4_t): vec4_t
            vec4_wrap2(v: vec4_t, min: vec4_t, max: vec4_t): vec4_t
        comparison:
            vec4_equals_exact(a: vec4_t, b: vec4_t): boolean
            vec4_equals(a: vec4_t, b: vec4_t, e: number = EPSILON): boolean
        string:
            vec4_str(v: vec4_t): string
            vec4_print(v: vec4_t): void
*/
