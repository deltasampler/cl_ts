...
export function cl_cam2_move_right(camera: cam2_t, dir: number) {
    cl_vec2_add_mul_s2(camera.position, camera.right, camera.movement_speed * dir);
}

export function cl_cam2_move_up(camera: cam2_t, dir: number) {
    cl_vec2_add_mul_s2(camera.position, camera.up, camera.movement_speed * dir);
}

export function cl_cam2_roll(camera: cam2_t, dir: number) {
    camera.roll += camera.roll_speed * dir;
}

export function cl_cam2_compute_proj(camera: cam2_t, viewport_x: number, viewport_y: number): mat4_t {
...
export function cl_cam2_proj_mouse(camera: cam2_t, mx: number, my: number, width: number, height: number): vec2_t {
